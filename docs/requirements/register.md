...
three defects in `apps/mobile-web/src/test-storage-guard.ts` itself, all posted on #329; not fixed on
`main` — a fix for all three is open, unmerged, and not yet reviewed-through in
[#333](https://github.com/NexusHero/Steuereule/pull/333) (Musti's own review of that fix found two
further one-line findings, addressed in a push at `3c9fd13`, still awaiting his re-read):
...
and asserts both reachable stores, that guard has three defects unfixed on `main` — a fix for all three
is open, unmerged, and not yet reviewed-through in [#333](https://github.com/NexusHero/Steuereule/pull/333)
...
gate, which is **not** cited as proof of the clause itself: it is open, draft, and Musti's review
currently reads it **refuted** with eight findings — a fix addressing all eight has been pushed
(`e3bc959`) and awaits his re-read.

**Not closed by this edit, again.** This corrects the register's evidence claim a second time; it does
not fix `apps/mobile-web/src/test-storage-guard.ts`. When Musti's three #329 findings and #331's eight findings are
