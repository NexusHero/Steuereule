...
| REQ-010 | Login/account endpoints carry baseline hardening: per-route login rate limiting, CSRF protection, `helmet`/CSP security headers, and a known-breached-password check at signup/change. | **Done** (CSRF · helmet/CSP · breach-check) · **not met (rate limiting)** — IP-keyed only, bypassable by a single-value `X-Forwarded-For`; closes only once the network property from [#292](https://github.com/NexusHero/Steuereule/issues/292) exists | Epic [#8](https://github.com/NexusHero/Steuereule/issues/8) · PR #116, #247, #350 | `apps/api/test/acceptance/req-010-security-hardening.test.ts` (rate-limit clause: proves IP-keyed counting, not failed logins — #248; #350 renamed the misleadingly-named "same account" case to what it actually proves — caller+path keying, not account identity — and added an explicit twelve-different-accounts discriminator alongside it), `apps/api/test/breach-check.test.ts`, `apps/api/test/cors.acceptance.test.ts`, `apps/api/test/acceptance/trusted-proxies-ip-resolution.test.ts` (real Postgres — #350 rewrote this file: the old A1 case's `@documents-defect #292` marker is dropped, since the single-value-header bypass it documented is now fixed in code; both describe blocks now prove the fix against a genuine second loopback peer, not a header value standing in for one — see Status accuracy — 2026-08-13 below), `apps/api/test/client-address.test.ts`, `apps/api/test/stamp-client-address.test.ts` (unit — #350's pure resolution function and its Fastify adapter, incl. a permanent control-proof of the `request.headers`-vs-`request.raw.headers` trap), `apps/api/test/trusted-proxies.test.ts` (unit), `apps/api/test/acceptance/req-010-rate-limit-ceilings.test.ts` (real Postgres — #350's ceiling raise: `customRules` matches proven per actually-reachable path, not the `/forget-password` literal `getDefaultSpecialRules()` names but this app never exposes), `apps/api/test/acceptance/auth-mount-cors.test.ts` (real Postgres — CORS on the `/api/auth/*` mount itself), `apps/api/test/acceptance/req-005-email-signup.test.ts:112-175` (nested `describe('the known-breach check (REQ-010, must land no later than REQ-005)')` — the breach-check clause, proven at acceptance tier against the real server: the HIBP call is stubbed and its hit count asserted, and the rejection asserts the exact `PASSWORD_COMPROMISED` code) |
...
## Status accuracy — 2026-08-13: #350/ADR-0035 closes the single-value-header bypass in code — REQ-010's Status/State cells are not re-read here, by design

`#350` (Musti's technical ruling, stakeholder's ceiling numbers) lands the IP-resolution seam
(`docs/adr/0035-ip-resolution-seam.md`): a Fastify `onRequest` hook stamps the real socket peer into a
self-owned header, overwriting whatever a caller sends, and better-auth's `ipAddressHeaders` is pointed
at that header exclusively. **This closes the exact bypass REQ-010's Status/State cells (and the
2026-08-05 section above) describe as gated on #292** — a rotating single-value `X-Forwarded-For` no
longer produces a fresh bucket per request, proven by breaking the fix itself (existence: delete the
stamp, the key reverts to a shared literal and an unrelated second peer's first attempt is `429`;
forgeability: trust an inbound value instead of overwriting, an attacker's chosen value becomes the
literal `RateLimit.key`) and by restoring it (both re-verify green).

**This section deliberately does NOT flip REQ-010's `Status`/`State` cells to `Done`, and does not
touch their wording.** Per ADR-0025 and Musti's own dispatch on this ticket: "the status cell is
Suhay's — REQ-010 is that ADR's own worked example." What #292/#277 still gate — and what the cells
above still correctly describe — is unchanged: whether a real deployment exists to *configure* a real
`TRUSTED_PROXIES` CIDR value at all, and the narrower residual case (a real proxy is configured, but
this origin is still reachable directly, bypassing it) that a network property, not a header resolver,
closes. What changed is narrower and more specific than either cell currently states: **the
single-value-header form of the bypass — the one A1 used to document as permanently unfixed — is now
closed independent of #292.** Whether that narrower fact is enough to move the Status/State wording,
and how, is the stakeholder's read, not this slice's.

**The `@documents-defect #292` marker is dropped from
`apps/api/test/acceptance/trusted-proxies-ip-resolution.test.ts`** (its old A1 case), because the
defect it named is fixed — leaving it in place would have `register-check`'s check 5 asserting an open
issue against a closed gap for the wrong reason (the issue really is still open, but this file no
longer documents an unfixed bypass; it now proves a fixed one, with a genuine second real peer rather
than a header value standing in for one). The two citation columns above (summary row and traceability
row) are updated to describe what the file now proves; the `Status`/`State`/GWT cells themselves are
left exactly as they were, per the paragraph above.

Also landed with #350: the rate-limit ceilings the stakeholder ruled on — `/sign-in*`, `/sign-up*`,
`/change-password`, `/change-email` raised 10s/3 → 10s/5; `/request-password-reset`,
`/send-verification-email` raised 60s/3 → 60s/5 (the window stays 60s — an earlier draft of the
ruling read "10s" for this group, which Musti's refinement caught as a 10× loosening on a
mail-sending path before anyone built it); `/get-session` untouched (100/10s, unchanged); `/verify-password`
(10s/3) and `/device` (60s/10) pinned at their current numbers — a destructive-action guard and a
guessable-code guard, neither in the question put to the stakeholder. All seven `customRules` keys
added are the app's actual reachable better-auth paths (`/sign-in/email`, `/sign-in/social`,
`/sign-up/email`, `/change-password`, `/change-email`, `/request-password-reset`,
`/send-verification-email`), not `getDefaultSpecialRules()`'s own `/forget-password` string — this
app's better-auth config exposes no endpoint at that path (the real endpoint is
`/request-password-reset`); a `customRules` key that never matches is silently inert (ADR-0021's own
shape), so the literal reachable paths are what's keyed, proven per path in
