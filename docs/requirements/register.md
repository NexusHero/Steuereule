...
| REQ-012 | Onboarding is joined to the real backend end to end: a brand-new guest's form prefills from `GET /v1/profile` (the honest all-null empty state, never mock data) and completing it persists through `PUT /v1/profile`. | **Done** | [#53](https://github.com/NexusHero/Steuereule/issues/53) | `apps/mobile-web/src/screens/onboarding/req-012-onboarding-vertical-join.test.tsx` — proves the `GET`/`PUT /v1/profile` vertical join; its no-client-storage assertion (`vi.spyOn(window.localStorage, 'setItem')` at :185) is **vacuous** — jsdom's `Storage` Proxy never routes through the spy, confirmed live with a same-line `localStorage.setItem('leak','steuerid')` that still passed the assertion (#323, Musti's repro) — so this file does **not** prove the "never written to localStorage/AsyncStorage/any client-side store" clause. That clause currently holds by static absence only (zero call sites for `localStorage`/`sessionStorage`/`AsyncStorage` across shipped `apps/*/src`, `packages/*/src`, measured 2026-08-10) and is unguarded against regression until #323 lands a control that can actually fail. See Status accuracy — 2026-08-10 below. |
| REQ-013 | A user can view and edit their stored profile (name, Steuer-ID, Steuernummer) against the live `GET`/`PUT /v1/profile`, with honest loading/empty/error states and no client-side persistence (ADR-0008). | **Done** | [#95](https://github.com/NexusHero/Steuereule/issues/95) · PRs #126, #149 | `apps/mobile-web/src/screens/ProfilScreen.test.tsx` (incl. the shared-cache regression), reachable via the tab bar since #149 — its no-client-storage assertion (`vi.spyOn(window.localStorage, 'setItem')` at :215-216) is **vacuous** for the same reason as REQ-012's (jsdom's `Storage` Proxy never routes through the spy; confirmed live, #323), so this file does **not** prove the "nothing sensitive is written to client storage (ADR-0008)" clause. That clause currently holds by static absence only (zero call sites for `localStorage`/`sessionStorage`/`AsyncStorage` across shipped `apps/*/src`, `packages/*/src`, measured 2026-08-10) and is unguarded against regression until #323 lands a control that can actually fail. See Status accuracy — 2026-08-10 below. |
...
| REQ-012 | Given a brand-new guest, when Onboarding mounts, then `GET /v1/profile` prefills the honest all-null empty state (never mock data), and completing the flow persists through `PUT /v1/profile`. | `apps/mobile-web/src/screens/onboarding/req-012-onboarding-vertical-join.test.tsx` — proves the vertical join only; its `localStorage` assertion is vacuous (jsdom Proxy defeats `vi.spyOn`, #323) | green — vertical-join clause only; the "never written to localStorage/AsyncStorage/any client-side store" clause is **unevidenced**, holding by static absence only until #323 lands a real control |
| REQ-013 | Given a stored profile, when the user opens Profil, then the live `GET /v1/profile` values render (honest loading/empty/error), edits save through `PUT`, and nothing sensitive is written to client storage (ADR-0008). | `apps/mobile-web/src/screens/ProfilScreen.test.tsx` — proves the `GET`/`PUT` view-and-edit round trip only; its `localStorage` assertion is vacuous (jsdom Proxy defeats `vi.spyOn`, #323) | green — view/edit clause only; the "nothing sensitive is written to client storage (ADR-0008)" clause is **unevidenced**, holding by static absence only until #323 lands a real control |
