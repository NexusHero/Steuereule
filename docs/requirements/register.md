...
by a bundled dependency. The shipped browser client of the `better-auth` npm package (not a file in
this repository) demonstrably calls one — `localStorage.setItem`, in the `broadcast-channel` module of
that package's own build output, around line 19 of its bundle, as installed. The honest bound was written into
