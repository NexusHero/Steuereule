...
4. **Secondary — `toContain(429)`, the "under" comment, arc42, no ADR.** All fixed: the account-keyed
   ...
   **ADR-0034** records the design decision (why account-keyed, why the hashed key, why 60s/5) and
   states the lockout residual explicitly as **open, pending the stakeholder's ruling** — not decided
   by this branch.

**Proposed status (awaiting the register owner's ruling — ADR-0025)**, not asserted here: the summary
table's Status cell above is left exactly as it read before this PR, on purpose — ADR-0025 reserves
that judgement to the register owner, who cannot certify evidence he cannot run, but does write what a
row means. The evidence above (and PR #339's own body) is offered for that ruling, not in place of it.
What a reader can independently verify today: **account-keyed rate limiting is proven** by a real,
positive, IP-rotation-surviving test, now covering both "it blocks eventually" and "it counts failures
only, and clears on success" (fixing the very gap this review found); **IP-keyed rate limiting remains
unproven as a control**, gated on #292 exactly as before, now with A0 as a second, independently
measured piece of evidence alongside A1. Whether that composite is written up as `Done`/`not met`, and
in what words, is the register owner's to decide — as is whether ADR-0034's still-open lockout residual
changes how "Done" should read for the account-keyed half.
