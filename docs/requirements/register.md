...
| REQ-010 | Login/account endpoints carry baseline hardening: per-route login rate limiting, CSRF protection, `helmet`/CSP security headers, and a known-breached-password check at signup/change. | **Done** (CSRF · helmet/CSP · breach-check, unchanged) · **rate limiting — composite ([#339](https://github.com/NexusHero/Steuereule/pull/339)):** **account-keyed clause meets REQ-010's own stated predicate** — repeated failed logins from the same account are rate-limited independent of the caller's IP, proven positively (not merely as a documented bypass) both in the acceptance suite and against the real compiled server — **but is not credited as plain `Done`: [ADR-0035](../adr/0035-account-keyed-login-rate-limit.md) names a residual and rules on none of five costed options.** Anyone who knows a victim's email can burn that victim's own quota with wrong passwords, locking them out even on their next, correct attempt; the mitigation choice (if any) is left **open for the stakeholder**. **IP-keyed clause stays `not met`, unchanged** — bypassable by a spoofed single-value `X-Forwarded-For` (A1) or by the shared no-header fallback newly measured as A0, closes only once the network property from [#292](https://github.com/NexusHero/Steuereule/issues/292) exists. See "Status accuracy — 2026-08-11" below for Robin's full evidence walkthrough this ruling is written against. | Epic [#8](https://github.com/NexusHero/Steuereule/issues/8) · PR #116, #247, #248, [#339](https://github.com/NexusHero/Steuereule/pull/339) | `apps/api/test/acceptance/req-010-security-hardening.test.ts` (rate-limit clause fixed by #248: sends a trusted origin so both the probe account and every attempt reach real credential validation; asserts the exact status sequence, not `toContain`), `apps/api/test/acceptance/trusted-proxies-ip-resolution.test.ts` (real Postgres — **A0**, new: no `X-Forwarded-For` at all collapses onto one shared bucket, `@documents-defect #292`; **A1**, revised to isolate the IP dimension with a fresh target account per attempt, `@documents-defect #292`; **the account-keyed control** (#248, ADR-0035), new, positive: the same full IP rotation as A1 but one fixed target account still trips 429, exact status sequence asserted — not `@documents-defect`, this is the fix; plus two new tests proving a success clears the bucket and never counts against it), `apps/api/test/login-rate-limit.test.ts` (unit — the pure key-derivation function, now a fixed-length sha256 digest), `apps/api/test/db-rate-limit.integration.test.ts` (real Postgres — `peekDbRateLimit`/`resetDbRateLimit`, the read-only-check/clear-on-success split), `apps/api/test/breach-check.test.ts`, `apps/api/test/cors.acceptance.test.ts`, `apps/api/test/trusted-proxies.test.ts` (unit), `apps/api/test/acceptance/auth-mount-cors.test.ts` (real Postgres — CORS on the `/api/auth/*` mount itself), `apps/api/test/acceptance/req-005-email-signup.test.ts:112-175` (nested `describe('the known-breach check (REQ-010, must land no later than REQ-005)')` — the breach-check clause, proven at acceptance tier against the real server: the HIBP call is stubbed and its hit count asserted, and the rejection asserts the exact `PASSWORD_COMPROMISED` code) |
...
| REQ-012 | Onboarding is joined to the real backend end to end: a brand-new guest's form prefills from `GET /v1/profile` (the honest all-null empty state, never mock data) and completing it persists through `PUT /v1/profile`. | **Done** | [#53](https://github.com/NexusHero/Steuereule/issues/53) | `apps/mobile-web/src/screens/onboarding/req-012-onboarding-vertical-join.test.tsx` — proves the `GET`/`PUT /v1/profile` vertical join. Its no-client-storage assertion no longer uses the vacuous `vi.spyOn` pattern: #329 (merged `66a481b`) replaced it with `assertNoStorageWrites()`/`stubStorageSetItem()` (`apps/mobile-web/src/test-storage-guard.ts`), a `vi.stubGlobal`-based guard the jsdom `Storage` Proxy cannot defeat, now asserting both `localStorage` **and** `sessionStorage` (`AsyncStorage` is not a dependency of this repo). **The guard is now sound, and the bundled-dependency gap it left open is now closed.** Musti's retroactive T1 review of #329 found three defects in `apps/mobile-web/src/test-storage-guard.ts`; all three are **fixed on `main`** as of `c351688` (#333, merged): (1) `assertNoStorageWrites` now checks `window[storeName].setItem === setItem` before trusting a zero call count, so a disconnected guard throws (naming the store) instead of passing quietly; (2) the stand-in is built from bound references to the real store's own methods rather than a `{ ...window[storeName] }` spread, so `getItem`/`removeItem`/`clear` delegate correctly instead of throwing; (3) `apps/mobile-web/vitest.config.ts`'s coverage `exclude` is now a `test-*.{ts,tsx}` glob rather than a hand-enumerated list. So this test proves both the vertical join and, through the now-fixed guard, no-client-storage for `localStorage` and `sessionStorage`. Independent of any test, the property also holds by **static absence** — zero first-party call sites for `localStorage`/`sessionStorage`/`AsyncStorage` across shipped (non-test) `apps/*/src`, `packages/*/src` (three matches, all comments, measured 2026-08-10) — which does not cover writes from bundled dependencies: the shipped browser client of the `better-auth` npm package (not a file in this repository) demonstrably calls `localStorage.setItem` — in `WindowBroadcastChannel.post()` — a write nothing in that class ever removes (its `setup()` only adds a `storage` listener), so this specific write is persistent, not transient. That gap is now closed: `e2424cc` (#331, merged) is a real-Chromium, real-stack gate — `e2e/storage/no-client-persistence.mjs`, wired into CI's `Browser gates` job — enumerating both `localStorage` and `sessionStorage` at five checkpoints (baseline before sign-in, mid-flow, after Onboarding save, Profil view, after Profil save) and asserting all five: baseline against a true empty set, every later checkpoint against baseline itself. Two stated limits, not oversights (both named in #331 itself): it is a **sampling** gate, so a write made and then removed between two checkpoints is unreached — a bound that still stands for any bundled-dependency write not identified here, distinct from `WindowBroadcastChannel.post()`'s persistent one above; and `AsyncStorage`/the native (iOS/Android) target are out of scope — a web-only real-browser observation. See Status accuracy — 2026-08-10 below. |
| REQ-013 | A user can view and edit their stored profile (name, Steuer-ID, Steuernummer) against the live `GET`/`PUT /v1/profile`, with honest loading/empty/error states and no client-side persistence (ADR-0008). | **Done** | [#95](https://github.com/NexusHero/Steuereule/issues/95) · PRs #126, #149 | `apps/mobile-web/src/screens/ProfilScreen.test.tsx` (incl. the shared-cache regression), reachable via the tab bar since #149 — proves the `GET`/`PUT` view-and-edit round trip. Its no-client-storage assertion no longer uses the vacuous `vi.spyOn` pattern: #329 (merged `66a481b`) replaced it with the same `assertNoStorageWrites()`/`stubStorageSetItem()` guard (`apps/mobile-web/src/test-storage-guard.ts`) described under REQ-012, now asserting both `localStorage` **and** `sessionStorage`. **The guard is now sound**, for the same reason as REQ-012's: Musti's retroactive T1 review of #329 found three defects in that shared helper — sharpest, it could not detect its own disconnection (passed after a real leak when handed an uninstalled `vi.fn()`) — all three are **fixed on `main`** as of `c351688` (#333, merged). Independent of any test, the property also holds by **static absence** — zero first-party call sites for `localStorage`/`sessionStorage`/`AsyncStorage` across shipped (non-test) `apps/*/src`, `packages/*/src` (three matches, all comments, measured 2026-08-10) — which does not cover writes from bundled dependencies: the shipped browser client of the `better-auth` npm package (not a file in this repository) demonstrably calls `localStorage.setItem` — in `WindowBroadcastChannel.post()` — a write nothing in that class ever removes (its `setup()` only adds a `storage` listener), so this specific write is persistent, not transient. That gap is now closed: `e2424cc` (#331, merged) is a real-Chromium, real-stack gate — `e2e/storage/no-client-persistence.mjs`, wired into CI's `Browser gates` job — enumerating both `localStorage` and `sessionStorage` at five checkpoints and asserting all five, baseline against a true empty set and every later checkpoint against baseline itself. Two stated limits, not oversights (both named in #331 itself): it is a **sampling** gate, so a write made and then removed between two checkpoints is unreached — a bound that still stands for any bundled-dependency write not identified here, distinct from `WindowBroadcastChannel.post()`'s persistent one above; and `AsyncStorage`/the native target are out of scope. See Status accuracy — 2026-08-10 below. |
...
`apps/mobile-web/src/screens/ProfilScreen.test.tsx` and `apps/mobile-web/src/screens/onboarding/req-012-onboarding-vertical-join.test.tsx`. Both now go through a shared
...
three defects in `apps/mobile-web/src/test-storage-guard.ts` itself, all posted on #329; all three are
now **fixed on `main`**, as of `c351688` (#333, merged):
...
3. `apps/mobile-web/vitest.config.ts`'s coverage `exclude` hand-enumerates test infrastructure and misses this file.
...
and asserts both reachable stores, that guard's three defects — including one that mirrored #323's own
class — are now fixed on `main` as of `c351688` (#333, merged), and the underlying property is
independently confirmed by static absence (`localStorage|sessionStorage|AsyncStorage` — three matches
across shipped (non-test) `apps/*/src`, `packages/*/src`, all comments, zero first-party call sites,
measured 2026-08-10; this does not cover writes from bundled dependencies, which only a real-page
observation reaches) and now, for the calibration half, by a merged real-browser gate: `e2424cc`
(#331), the record of which is below in the 2026-08-10 merge re-read.
...
#331 is resolved, both merged, these four cells are due a third and hopefully final re-read, at which point the
"open defects" language should retire in favour of citing the fixed, calibration-proven controls
directly.

## Status accuracy — 2026-08-10: #333 and #331 merged — the third re-read, done on merge

This is that third re-read, on the trigger it was promised on: a merge already obliges one (Musti's
ruling on #334). Two merges landed on `main`:

- **`c351688`** — #333 merged. All three defects Musti's retroactive T1 review of #329 found in
  `apps/mobile-web/src/test-storage-guard.ts` are fixed: the identity check on
  `window[storeName].setItem` lets the guard detect its own disconnection, the stand-in is built from
  bound references to the real store's methods instead of a `{ ...window[storeName] }` spread (so
  `getItem`/`removeItem`/`clear` delegate instead of throwing), and the coverage `exclude` is a
  `test-*.{ts,tsx}` glob instead of a hand-enumerated list.
- **`e2424cc`** — #331 merged. `e2e/storage/no-client-persistence.mjs` is on `main` and wired into
  CI's `Browser gates` job.

**The section above ("2026-08-10 (re-read): #329 landed the control, the control has open defects")
is left as written where it narrates what Musti found and when** — that record is still accurate. Its
handful of sentences that named the fix's state as of that morning's re-read (open/unmerged/not yet
reviewed-through in #333; #331 open, draft, not cited) have been corrected in place, in that section
and in the REQ-012/REQ-013 summary-table and traceability-table cells above and below, because those
particular sentences were the ones designed to change only on merge (Musti's own framing) — not narrative being
rewritten, but the self-invalidating clause the narrative always pointed at doing exactly what it was
built to do.

**The substantive change, and the reason this re-read matters beyond bookkeeping.** F3 of Musti's
review on #334 (before either merge) established that the register's ADR-0008 claim — "nothing
sensitive is written to client storage" — rested on a static-absence grep, and a grep of first-party
call sites is a mechanism check standing in for an effect check (ADR-0028): it cannot see a write made
by a bundled dependency. `better-auth`'s client bundle (an npm package, not a file in this repository)
demonstrably writes to `localStorage` in `WindowBroadcastChannel.post()` — a symbol name rather than a
path, deliberately: a path is fine as evidence in review prose, but wrong as a citation in a document
a resolver reads, and a symbol survives a version bump that moves the file or shifts the line in a way
a coordinate does not. The honest bound was written into
the register at the time, and the artefact that would actually close it, #331, was open and correctly
not cited as evidence while it was under refutation.

**#331 is now merged, reviewed through, and green in CI, so REQ-012/REQ-013's ADR-0008 evidence is
genuinely stronger, not just re-labeled.** What the gate observes: a real-Chromium, real-stack run
(`e2e/storage/no-client-persistence.mjs`, in CI's `Browser gates` job) that enumerates both
`window.localStorage` and `window.sessionStorage` at five checkpoints across the two flows that matter
(fresh-account Onboarding through save; Profil view → edit → save) — baseline before sign-in, mid-flow,
after the Onboarding save, Profil view before any edit, and after the Profil save — and asserts all
five: baseline against a true empty set, every later checkpoint against baseline itself, so any new key
or changed value since baseline is a named finding. This reaches past the first-party source tree into
whatever the served bundle actually does, including `better-auth`'s own writes, closing exactly the gap
F3 named.

**One further fact worth recording, because it sharpens what "closes exactly the gap" means for this
write specifically.** `WindowBroadcastChannel.post()`'s write is never removed: its class's `setup()`
only adds a `storage` event listener, nothing else in it calls `removeItem`/`clear`. So the one
bundled-dependency write identified here is persistent, not transient — it sits in `localStorage` at
rest, which is exactly the shape #331's checkpoint-diff catches (a key present at any post-baseline
checkpoint that wasn't present at baseline), not the shape its sampling limit is stated against below
(a write made and then removed *between* checkpoints). This is a claim about this one identified write,
not about bundled writes generally — the sampling bound immediately below still stands for any
bundled-dependency write not identified here.

What it still does not reach, stated in #331 itself rather than discovered as a gap later: it is a
**sampling** gate, so a write made and then removed between two checkpoints (e.g. a value sitting in
`localStorage` for the few hundred milliseconds a field is being typed into) would not appear in any
dump — a decided limit (ADR-0028 rules against wrapping `Storage.prototype.setItem` to catch this, as
that aims at the mechanism upstream of the hazard rather than the hazard itself, which is data at
rest). `AsyncStorage` and the native (iOS/Android) target are out of scope — this is a web-only
real-browser observation. Both are stated bounds, not oversights, and both are carried into the
summary and traceability cells above rather than left implicit.

**Status stays `Done` on both REQ-012 and REQ-013.** Nothing here changes delivered behaviour; this
strengthens the evidence column from "static absence plus an open, uncited real-browser gate" to
"static absence plus a merged, CI-gated real-browser gate that reaches bundled-dependency writes,"
with its own stated sampling and platform-scope limits carried alongside it.

## Status accuracy — 2026-08-11: #248 fixed, an account-keyed rate-limit control lands, IP-keyed clause stays not met

Requested: establish what the rate-limit bucket actually is today (measured, not read from config),
design what can close it without waiting on #292's real deployment topology, and fix #248's own
misleadingly-named test. All three are done; the register is corrected to match.

**What was measured, against the real stack, not assumed.** Two describe blocks in
`apps/api/test/acceptance/trusted-proxies-ip-resolution.test.ts` now prove two distinct failure
shapes that were previously conflated under one "IP-keyed, bypassable" label:

- **A0 (new).** A caller who sends **no** `X-Forwarded-For` header at all — the actual shape of
  every request in this project's own CI, since no reverse proxy sits in front of `node --import tsx
  dist/main.js` in the `smoke`/`Browser gates` jobs — makes `getIp()` return `null`. better-auth's own
  fallback for that is a single literal key shared by every caller, not a fresh one per caller. Proven
  two ways: inside the vitest suite (`NODE_ENV=test` forces better-auth's own dev/test fallback to a
  fixed `127.0.0.1`, since `@better-auth/core`'s env module reads `NODE_ENV` into a top-level `const`
  exactly once per process and no later `process.env` edit can undo that within the same worker) and,
  independently, against a real compiled boot with `NODE_ENV` genuinely unset (this session's own
  manual boot-proof, matching the `smoke`/`Browser gates` shape exactly) — where the key is the literal
  `no-trusted-ip|/sign-in/email` string better-auth's own code produces, capped at 3 and shared across
  two entirely unrelated target accounts probed alternately. This is the literal mechanism behind the
  "Rate limiting could not determine a client IP..." warning logged on every CI run this week, and the
  reason `Browser gates`' independent e2e scripts observably collided on each other's quota (Salih's
  finding, cited in the task that opened this work). `@documents-defect #292` — closing it needs a real
  reverse proxy reliably stamping `X-Forwarded-For`, which needs the deployment #292 still doesn't have.
- **A1 (revised).** Unchanged in substance — a rotated single-value `X-Forwarded-For` still buys a
  fresh IP-keyed bucket every request — but now isolated with a fresh, never-repeated target account
  per attempt (the realistic shape of stolen-credential-list stuffing: many accounts, one guess each),
  so it measures the IP dimension alone rather than being entangled with the new account-keyed control
  below. Still `@documents-defect #292`, still gated the same way.

**What was built, and why it does not need #292.** `apps/api/src/auth/login-rate-limit.ts` adds a
second, independent limiter — better-auth's own top-level `hooks.before` — keyed on the **target
account** (`login-attempts|<normalized-email>`, 60s/max 5) rather than the caller's claimed IP, reusing
the same DB-backed `consumeDbRateLimit` algorithm every other in-process limiter in this codebase
already uses (`apps/api/src/auth/db-rate-limit.ts`, ADR-0013 §6's own instruction — no new mechanism). An attacker who
fully defeats the IP-keyed limiter (a fresh `X-Forwarded-For` on every request, exactly A1's shape)
still cannot rotate the email address they are guessing passwords for — proven directly, both in the
acceptance suite and by hand against the real compiled server (`node --import tsx dist/main.js`, no
`NODE_ENV` set): 7 sign-in attempts against one account, a fresh `X-Forwarded-For` on every one, `401`
for the first 5 and `429` from the 6th; `RateLimit` rows afterward show eight distinct
single-count `<ip>|/sign-in/email` rows (the IP-keyed bucket, exactly as bypassed) alongside one
`login-attempts|<email>` row at count 5 (the account-keyed bucket, capped). This closes REQ-010's own
GWT wording — "repeated failed logins from the **same account**" — without any dependency on #292 or
#277's still-undetermined CIDR values, because it never asks "is this IP real."

**What this does NOT close, stated plainly rather than left to be discovered later.** The account-keyed
control has a threat model, not a blanket fix: it stops sustained guessing against **one** known
account regardless of source IP, but it cannot help against a genuine credential-stuffing run that
tries many different, attacker-known (breached) email+password pairs **once each** — that shape defeats
both limiters equally and is a different attack than the one REQ-010's GWT names. It also does not make
`Session.ipAddress` or the IP-keyed bucket itself trustworthy — Musti's ADR-0021 control-test finding
(`apps/mobile-web/src/screens/device/useDeviceSessions.ts`'s own header comment: configuring
`trustedProxies` for a single-value header removes the check rather than replacing it, since there is
nothing to its right to strip) stands unchanged, and this design deliberately does not attempt to work
around it — a rate limiter keyed on a spoofable address would be worse than an honestly-global one,
because it would look like a control. Closing the IP-keyed clause for real still needs #292's real
deployment and #277's real CIDR ranges, in that order — neither is touched by this change.

**#248 fixed.** `apps/api/test/acceptance/req-010-security-hardening.test.ts`'s rate-limit test sent no
`origin` header, so both the probe sign-up and all 12 sign-in attempts were rejected by better-auth's
own origin check (403 `MISSING_OR_NULL_ORIGIN`) before ever reaching credential validation — the `429`
the old assertion found came from the router-level limiter counting origin-rejected requests, not from
12 genuine wrong-password attempts. Fixed per #248's own prescribed remedy (option 1: send a trusted
origin, matching `apps/api/test/acceptance/trusted-proxies-ip-resolution.test.ts`'s A1/A2 pattern) — the account now genuinely
exists and every attempt reaches real credential validation; the assertion is the exact status sequence
(`[401,401,401,429,429,...]`), not `toContain(429)`. **Red path run and confirmed (ADR-0021):**
reverting the fix's two `origin` headers reproduces the original defect exactly — the sign-up itself
returns 403 and the very first assertion (`signUpResponse.status` to be `200`) fails with "expected 403
to be 200", not a downstream symptom.

**Control proofs for the new account-keyed limiter (ADR-0021, both branches):**
- *Existence* — the `hooks: { before: createLoginRateLimitBeforeHook(prisma), after:
  createLoginRateLimitAfterHook(prisma) }` wiring in `apps/api/src/auth/better-auth.ts` removed
  entirely: the account-keyed describe block's 7-attempt run returns `401` seven times, no
  `429` ever (confirmed against the real IP-rotation test, not merely reasoned about).
- *Validity* — `LOGIN_RATE_LIMIT_MAX` corrupted to a value the test's own fixed-length attempt run
  cannot reach: the same describe block goes red for the same reason, no `429` within the attempts made.
- Both restored; the suite returns to green.

### §4 review (Musti, PR #339) — three blocking findings, all fixed; evidence below (Robin's, per ADR-0025)

Musti refuted the draft above on first read. All three blocking findings are fixed on this branch;
the evidence for each — what a reader can re-run — belongs here. The **status** conclusion two
paragraphs below is left for the register owner, unchanged in shape from before this review.

1. **The limiter counted attempts, not failures, and never reset.** Fixed: the control now runs across
   *two* better-auth hook phases instead of one. `hooks.before` (`createLoginRateLimitBeforeHook`)
   only ever peeks at the current count (`peekDbRateLimit`, `apps/api/src/auth/db-rate-limit.ts`) and
   blocks if already over quota — it never writes. `hooks.after` (`createLoginRateLimitAfterHook`)
   inspects the real outcome (`ctx.context.returned`, an `APIError` on failure, the session payload on
   success) and only then writes: a failure increments the bucket, a success clears it outright
   (`resetDbRateLimit`). Two new acceptance tests in the account-keyed describe block prove this: six
   correct sign-ins in a row never trip 429 and leave no row behind; two failed attempts followed by
   one correct one reset the count to nothing rather than merely lowering it. Both were run against a
   deliberately-reverted version of the fix (ADR-0021) and went red for exactly the reason named.
2. **An unbounded, attacker-controlled key.** Fixed: `loginRateLimitKey` now hashes the normalized
   email (`login-attempts|` + sha256 hex, 79 chars total, always) instead of embedding it verbatim.
   `apps/api/test/login-rate-limit.test.ts` asserts the fixed length against both a 5000-character
   pseudo-email and a 5000-character non-email junk string, and was run against a reverted (unhashed)
   version to confirm it goes red. An index/pruning story for `RateLimit.key` itself was deliberately
   **not** folded into this fix, per Musti's own request — filed as
   [#342](https://github.com/NexusHero/Steuereule/issues/342).
3. **A non-deterministic acceptance-test query.** Fixed: A0's and A2's key assertions now `findMany` +
   assert the complete set of distinct keys, rather than an unordered `findFirst` with a `contains`
   filter that could return a row the test never created. Both describe blocks gained a `beforeEach`
   cleanup (not only `afterEach`) so the precondition is established, not assumed — the same table is
   shared with every other file/process talking to the same database, and nothing else prunes it.
   Reproduced Musti's exact repro directly: seeded a stale `no-trusted-ip|/sign-in/email` row by hand,
   confirmed the fixed test passes regardless, then removed the new `beforeEach` and confirmed the
   same seeded row reproduces his exact failure (`expected [...] to deeply equal ['127.0.0.1|/sign-in/
   email']` — got both keys back) before restoring the fix.
4. **Secondary — `toContain(429)`, the "under" comment, arc42, no ADR.** All fixed: the account-keyed
   test now asserts the exact `[...401×MAX, 429, 429]` sequence; A2 now sends
   `SIGN_IN_WINDOW_MAX + 1` attempts (not `+ 2`) and pins the isolation margin with
   `expect(LOGIN_RATE_LIMIT_MAX).toBeGreaterThan(ATTEMPTS)` so a silent retune of either constant fails
   loudly instead of passing for the wrong reason; `docs/arc42/README.md`'s cross-cutting section now
   describes both limiters and is honest that only the account-keyed one is effective today;
   **ADR-0035** records the design decision (why account-keyed, why the hashed key, why 60s/5) and
   states the lockout residual explicitly as **open, pending the stakeholder's ruling** — not decided
   by this branch.

**Ruling (register owner).** The summary table's Status cell and this row's State cell above are now
corrected to a composite; this paragraph replaces the placeholder that stood in their place through
review. I write meaning, not evidence — everything measured above is Robin's and Musti's, re-run and
confirmed by each other; what follows is what it means for the row.

**The account-keyed clause is not left `not met`.** The evidence above is a positive,
IP-rotation-surviving proof that repeated failed logins against one account are rate-limited, plus a
separate positive proof that a successful sign-in never counts and clears any prior count. Together
that is REQ-010's own stated predicate — "repeated failed logins from the same account" — genuinely
met, independent of the deployment gap #292 still names. Writing `not met` against that evidence would
misstate real, verified work, in the opposite direction from the defect this row exists to correct.

**The account-keyed clause is equally not marked plain `Done`.** ADR-0035, written on this branch,
names a residual this design does not and structurally cannot close by itself: anyone who knows a
victim's email can exhaust that victim's own quota with wrong passwords from any IP, at any time,
locking the legitimate holder out on their next attempt even with the correct password. Five
mitigations are costed there (do nothing / compound account+IP key / exponential backoff / CAPTCHA
step-up / notify-on-lockout) and **none is chosen** — a stakeholder decision, not Robin's or mine to
make by omission or by silence. An unqualified `Done` would tell a reader the sign-in path carries no
known lockout exposure, which is false as of this ruling.

So: **account-keyed rate limiting — green (acceptance) against its own stated predicate, qualified:
full `Done` credit withheld while ADR-0035's lockout residual is unruled.** **IP-keyed rate limiting —
stays `not met`, unchanged**, now with A0 (the shared no-header fallback) alongside A1 as independently
measured evidence of the same underlying gap, still closing only once #292's real deployment exists.

This composite follows the same shape already used elsewhere in this register (REQ-008 Google/Apple,
REQ-009 web/Expo, and REQ-010's own prior split from the 2026-08-03 reconciliation) — applied one level
deeper, within a single clause rather than across two platforms. It moves to plain `Done` the moment
the stakeholder rules on ADR-0035's open question and, if that ruling changes the mechanism, once the
resulting evidence is re-cited here.

**Not this register's call, named so it isn't lost:** whether the account-keyed limiter's own residual
should itself carry a documents-defect-style regression test — so it stays visibly proven-open the way
A1/A0 do for the IP-keyed clause — is a testing-design question for Musti/Salih, not a register-meaning
one.

## Status accuracy — 2026-08-11: REQ-016 does not exist yet — #321 read, judged not ready

Musti flagged the same gap twice, independently (F8 on #340, F7 on #341): `REQ-016` was being cited in
both PRs while the register stops at REQ-015. REQ-015's own row already names why — Segment 2 of
ADR-0031's interview "gets its own REQ once its ticket (#321) is refined." Both PRs have since removed
the premature citation and now cite #321 alone, which is correct and should stay that way. I read #321
in full, ADR-0031, and both PRs' complete review threads before ruling.

**Ruling (register owner): #321 is not refined enough to write a testable REQ-016 GWT today, and I am
not creating the row.** An unrefined REQ that exists is worse than an absent one — the next dev would
have something to cite that describes intent, not a checkable acceptance criterion, which is exactly
the class of drift ADR-0025 exists to stop. Specifically, by #321's own Definition of Ready checklist:

- **The Given–When–Then itself is unchecked**, and #321's text says why: it is "unblocked" by two
  recent rulings but still "needs drafting **at joint refinement**" — that is this register's own
  grilling step (Suhay + Musti, ADR-0018), and it has not happened for this ticket. Writing a GWT here
  now, outside that step, would be doing the refinement alone instead of jointly, which is the exact
  failure mode the process is built to avoid.
- **The register row itself is an unchecked DoR item on #321**, listed as "its own REQ alongside #318's
  REQ-015" — i.e. #321 already expects this ruling to land through refinement, not ahead of it.
- **"Where 'Vormerken' intents live" is unresolved** — an open data-modelling question (whether a gate's
  recorded interest is an `InterviewAnswer` row or its own model) that a GWT about persistence cannot
  responsibly paper over.
- **`partner`'s scope was ruled the same day this section was written (2026-08-11)**: full build,
  including tax-year coupling (ADR-006) and a partner-invitation flow — a materially larger slice than
  the ticket carried until today. #321's own text flags that "Musti's technical half of the refinement
  needs to size this explicitly before this ticket's one-PR shape can be trusted." A REQ row written
  before that sizing would be describing a shape not yet confirmed buildable as scoped.
- **The tariff strand's published-value provenance is a named, currently blocking gap** (#340 F1,
  still draft, no primary-source figures reachable from this environment — network access is the
  stakeholder's to unblock, not a register question). Any REQ-016 acceptance criterion touching
  `incomeTax`/`veranlagungsartenvergleich` cannot honestly claim correctness against published values
  while F1 stands — a constraint on wording I'd have to apply the moment I write one, so it is cleaner
  to wait until F1 is resolved than to write a criterion around a hole.
- **The engineering ADR this work traces to — Musti's Veranlagungsartenvergleich decision, currently on
  [PR #343](https://github.com/NexusHero/Steuereule/pull/343) — is itself still unmerged**, and its
  number is not yet safe to cite: it collided with Robin's rate-limit ADR on this very branch and moved
  to ADR-0035, so `docs/adr/0034-*.md` doesn't exist yet and may not land as `0034` at all. Cited here
  as `#343` rather than by a not-yet-resolvable id, same convention Musti used for his own draft and
  asked Robin to use for `REQ-016` itself — accurate today, swapped for the real id once #343 merges.
  Either way, the contract a REQ-016 GWT would cite doesn't yet exist on `main`.

None of this is a criticism of the work in #340/#341 — the entry-layer seam (R1) and the tariff module
(N1/N2) are both real, well-controlled `packages/core` slices, reviewed at T1 depth with genuine
red/green control proofs. What is missing is specifically the register-row precondition: a jointly
refined, testable GWT, plus two named scope/evidence gaps that bear directly on what such a GWT could
honestly say. **REQ-016 gets created the moment #321's GWT is drafted at joint refinement and the
`partner`-sizing and provenance items above are no longer open** — this section is the record of why it
isn't today, so the next person who reaches for `REQ-016` finds a reason instead of a guess.
...
| REQ-010 | Given repeated failed logins from the same account/origin, when a configured threshold is exceeded, then further attempts are rate-limited rather than unbounded. Two independent limiters now implement this, keyed on different things, and only one of them is trustworthy against a caller who controls their own network headers: **(a) the account-keyed limiter (#248, `apps/api/src/auth/login-rate-limit.ts`, design in ADR-0035)** keys on a fixed-length hash of the target email (`login-attempts\|sha256(email)`, 60s/max 5), split across two better-auth hooks — `hooks.before` only ever peeks and blocks, ahead of both the endpoint's own origin check and credential validation; `hooks.after` is the only place it writes, once the real outcome is known: a **failed** attempt counts, a **successful** one clears the bucket outright — an attacker cannot rotate the account they are guessing passwords for the way they can rotate a header, so this closes "repeated failed logins from the same account" **today**, independent of IP, at the cost of not helping against a stolen-credential list tried once per account (a different threat this REQ does not name), and a stated-open lockout residual (an attacker who knows the victim's email can still burn that account's quota with deliberate wrong passwords — ADR-0035's Consequences, awaiting a stakeholder ruling). **(b) better-auth's own built-in limiter** keys on `${ip}\|${path}` (`createRateLimitKey`) and **holds only as long as the caller's IP matches the real socket peer** — `getIp()` returns a **single-value** `X-Forwarded-For` verbatim regardless of `advanced.ipAddress.trustedProxies`: whoever sends a new value per request gets a new bucket per request (**A1 — documented, not fixed; the test is green *because* the bypass exists**), and a caller who sends **no** `X-Forwarded-For` at all — today's real CI/local topology, no reverse proxy in front — collapses onto a single literal fallback key shared by every caller regardless of target account (**A0 — measured directly against the real stack, both under `NODE_ENV=test`'s `127.0.0.1` fallback and, independently, a real boot with `NODE_ENV` genuinely unset, where the key is the literal `no-trusted-ip\|/sign-in/email` better-auth's own code produces — the exact mechanism behind the "Rate limiting could not determine a client IP..." warning logged on every CI run, and the reason `Browser gates`' independent e2e scripts collided on each other's quota, Salih's finding**). `trustedProxies` closes the **multi-hop** form (A2/A3); neither single-value form is closed by any `TRUSTED_PROXIES` value — only a network property (the app unreachable except through the real proxy) closes it, and that does not yet exist ([#292](https://github.com/NexusHero/Steuereule/issues/292), successor to the closed #246: still no deployment pipeline, and consequently no production `TRUSTED_PROXIES` value either); given a state-changing request to an auth endpoint, when it lacks a valid CSRF token/origin check, then it is rejected; given any API response, when inspected, then it carries `helmet`-set security headers and a CSP disallowing inline/unsafe script; given a new password at signup/change, when it matches a known-breached-password list, then it is rejected with a clear message before acceptance. *Traces to: engineering ADR-0007's "Security hardening" section; the 2026-07-23 decision list.* | `apps/api/test/acceptance/req-010-security-hardening.test.ts` (rate-limit clause fixed, #248: sends a trusted origin on the sign-up and every sign-in attempt, so the probe account is real and each attempt reaches credential validation; asserts the exact `[401,401,401,429,429,...]` sequence), `apps/api/test/acceptance/trusted-proxies-ip-resolution.test.ts` (real Postgres — **A0** new, `@documents-defect #292`, `findMany`+distinct-key assertion with a `beforeEach` table wipe, not an unordered `findFirst`; **A1** revised to isolate the IP dimension with one fresh target account per attempt, `@documents-defect #292`; **the account-keyed control** new and positive, not `@documents-defect` — full IP rotation, one fixed account, exact status sequence asserted, plus two tests proving a success clears the bucket and never counts against it; **A2** now sends `SIGN_IN_WINDOW_MAX + 1` attempts with its isolation margin pinned by assertion, not narrated), `apps/api/test/login-rate-limit.test.ts` (unit — fixed-length key assertions, including a 5000-char junk input), `apps/api/test/db-rate-limit.integration.test.ts` (real Postgres — `peekDbRateLimit`/`resetDbRateLimit`), `apps/api/test/breach-check.test.ts`, `apps/api/test/trusted-proxies.test.ts` (unit), `apps/api/test/acceptance/auth-mount-cors.test.ts` (real Postgres), `apps/api/test/acceptance/req-005-email-signup.test.ts:112-175` (breach-check clause, proven at acceptance tier against the real server: the HIBP call is stubbed and its hit count asserted, and the rejection asserts the exact `PASSWORD_COMPROMISED` code) | green (acceptance): CSRF, Header/CSP, Breach · **rate limiting — composite:** account-keyed clause **green (acceptance)** against REQ-010's own stated predicate — proven positively, not merely as a documented bypass — **qualified: full `Done` credit withheld while ADR-0035's lockout residual is unruled by the stakeholder** · IP-keyed clause **stays `not met`** — A1/A0 both green *as evidence of the bypass*, unchanged, gated on #292 |
...
| REQ-012 | Given a brand-new guest, when Onboarding mounts, then `GET /v1/profile` prefills the honest all-null empty state (never mock data), and completing the flow persists through `PUT /v1/profile`. | `apps/mobile-web/src/screens/onboarding/req-012-onboarding-vertical-join.test.tsx` — proves the vertical join; its `localStorage`/`sessionStorage` assertion now runs through `assertNoStorageWrites()`/`stubStorageSetItem()` (#329, merged `66a481b`), which the jsdom `Storage` Proxy cannot defeat — the three defects that guard carried (Musti's retroactive T1 review of #329), the sharpest being that it could not detect its own disconnection, are fixed on `main` as of `c351688` (#333, merged) | green — vertical-join clause proven; the "never written to localStorage/AsyncStorage/any client-side store" clause is **proven** by a real guard whose three defects are fixed on `main` as of `c351688` (#333, merged), corroborated by static absence (zero first-party call sites, measured 2026-08-10 — does not cover bundled-dependency writes) and closed for that bundled-dependency gap by a real-browser gate, `e2424cc` (#331, merged, wired into CI's `Browser gates` job) — five checkpoints across both stores, baseline against a true empty set, every later checkpoint against baseline itself; #331 is a stated sampling gate (a write made and removed between checkpoints is unreached) and does not cover `AsyncStorage`/the native target |
| REQ-013 | Given a stored profile, when the user opens Profil, then the live `GET /v1/profile` values render (honest loading/empty/error), edits save through `PUT`, and nothing sensitive is written to client storage (ADR-0008). | `apps/mobile-web/src/screens/ProfilScreen.test.tsx` — proves the `GET`/`PUT` view-and-edit round trip; its `localStorage`/`sessionStorage` assertion now runs through the same `assertNoStorageWrites()`/`stubStorageSetItem()` guard (#329, merged `66a481b`), the jsdom `Storage` Proxy cannot defeat it — the three defects that guard carried (Musti's retroactive T1 review of #329), the sharpest being that it could not detect its own disconnection, are fixed on `main` as of `c351688` (#333, merged) | green — view/edit clause proven; the "nothing sensitive is written to client storage (ADR-0008)" clause is **proven** by a real guard whose three defects are fixed on `main` as of `c351688` (#333, merged), corroborated by static absence (zero first-party call sites, measured 2026-08-10 — does not cover bundled-dependency writes) and closed for that bundled-dependency gap by a real-browser gate, `e2424cc` (#331, merged, wired into CI's `Browser gates` job) — five checkpoints across both stores, baseline against a true empty set, every later checkpoint against baseline itself; #331 is a stated sampling gate (a write made and removed between checkpoints is unreached) and does not cover `AsyncStorage`/the native target |
