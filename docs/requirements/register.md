...
| REQ-010 | Login/account endpoints carry baseline hardening: per-route login rate limiting, CSRF protection, `helmet`/CSP security headers, and a known-breached-password check at signup/change. | **Done** (CSRF · helmet/CSP · breach-check · **account-keyed rate limiting**, #248) · **not met (IP-keyed rate limiting)** — the caller's IP is bypassable/collides regardless of `TRUSTED_PROXIES`: a rotated single-value `X-Forwarded-For` buys a fresh bucket per request (A1), and no `X-Forwarded-For` at all (today's real CI/local topology — no reverse proxy) collapses every caller onto one shared bucket (A0, measured directly, both live only until the network property from [#292](https://github.com/NexusHero/Steuereule/issues/292) exists). The account-keyed control does not depend on either: it closes the requirement's own "repeated failed logins from the same account" clause today, independent of caller IP, by keying on the account being attacked instead — see the traceability row for the exact mechanism and its stated limit (does not help against a stolen-credential list, one guess per account) | Epic [#8](https://github.com/NexusHero/Steuereule/issues/8) · PR #116, #247, #248 | `apps/api/test/acceptance/req-010-security-hardening.test.ts` (rate-limit clause fixed by #248: sends a trusted origin so both the probe account and every attempt reach real credential validation; asserts the exact status sequence, not `toContain`), `apps/api/test/acceptance/trusted-proxies-ip-resolution.test.ts` (real Postgres — **A0**, new: no `X-Forwarded-For` at all collapses onto one shared bucket, `@documents-defect #292`; **A1**, revised to isolate the IP dimension with a fresh target account per attempt, `@documents-defect #292`; **the account-keyed control** (#248), new, positive: the same full IP rotation as A1 but one fixed target account still trips 429 — not `@documents-defect`, this is the fix), `apps/api/test/login-rate-limit.test.ts` (unit — the pure key-derivation function), `apps/api/test/breach-check.test.ts`, `apps/api/test/cors.acceptance.test.ts`, `apps/api/test/trusted-proxies.test.ts` (unit), `apps/api/test/acceptance/auth-mount-cors.test.ts` (real Postgres — CORS on the `/api/auth/*` mount itself), `apps/api/test/acceptance/req-005-email-signup.test.ts:112-175` (nested `describe('the known-breach check (REQ-010, must land no later than REQ-005)')` — the breach-check clause, proven at acceptance tier against the real server: the HIBP call is stubbed and its hit count asserted, and the rejection asserts the exact `PASSWORD_COMPROMISED` code) |
...
- *Existence* — the `hooks: { before: createLoginRateLimitHook(prisma) }` wiring in `apps/api/src/auth/better-auth.ts`
  removed entirely: the account-keyed describe block's 7-attempt run returns `401` seven times, no
...
**Ruling (register owner): REQ-010's rate-limiting clause moves off a single "not met" label to a
composite one**, the same shape already used for its own CSRF/headers/breach-check split and for
REQ-008/REQ-009 elsewhere in this register: **account-keyed rate limiting is Done**, cited against its
own real, positive, IP-rotation-surviving test; **IP-keyed rate limiting stays not met**, gated on #292
exactly as before, now with A0 as a second, independently-measured piece of evidence alongside A1
rather than A1 standing alone. Nothing here overstates the fix — the requirement's plain-language
promise ("repeated failed logins from the same account ... rate-limited rather than unbounded") is now
true today, independent of network topology; the IP-keyed half's spoofability is unchanged and still
requires #292/#277.
