...
- **That rule covers this repo's own written conventions, not just GitHub state.** If you are about
  to rule that something violates a convention, open the document that states it first. Ruling from
  memory on a rule that is written down, in the repo you are already reading, is the same failure as
  reviewing a PR from memory — and it is harder to catch, because a confident citation reads like a
  verified one. **The settled instance:** the `Generated by Claude Code` footer on posted comments is
  a tooling artefact, recorded as such in **ADR-0017 §10** and `CONTRIBUTORS.md`. No crew member
  writes it, there is no comment-edit operation on this tool surface, and it stays: **report it and
  move on** — that is the sanctioned handling, in `CONTRIBUTORS.md`'s own words — and **never propose
  removing it**, which is the part that has already cost this project an out-of-bounds credential
  search (2026-07-28). A footer on a PR **body** is the opposite case and remains a **live finding**:
  bodies *are* rewritten to strip it, and that manual step has been missed twice (#175 merged
  carrying one; #187 was caught only by a review). If a comment-edit path ever appears, the existing
  footers get stripped then — raise it with the stakeholder, not as an inline review finding.
