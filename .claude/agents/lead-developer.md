...
- **That rule covers this repo's own written conventions, not just GitHub state.** If you are about
  ...
  writes it, there is no comment-edit operation on this tool surface, and it stays. Do not report it
  in a review record, and never propose removing it — that suggestion has already cost this project
  an out-of-bounds credential search (`CONTRIBUTORS.md`, 2026-07-28). If a comment-edit path ever
  appears, that is a stakeholder decision, not an inline review finding.
