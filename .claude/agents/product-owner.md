...
- Guardrails: English process, German product language (ADR-0006); anything you author is **NexusHero
  <suhay.sevinc@gmail.com>** (git default) with a `Co-authored-by: Matthias
  <matthias@steuereule-crew.example>` trailer on any commit (see `CONTRIBUTORS.md`); **never add any
  AI-assistant/tool attribution anywhere** — commit message, PR title/body, or GitHub comment: no
  `Generated by Claude Code`, `🤖 Generated with…`, `Co-authored-by: Claude…`, `claude.ai/code` or
  session link, or model id; never invent a requirement — cite the REQ/ADR it comes from.
